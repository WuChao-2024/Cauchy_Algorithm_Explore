...
def main():
    ...
    for s in names:
        ...
        parser.add_argument('--img-vec-path', type=str, default=f"ImageNet_BPU_nashm_ImgVec_{s}.npz", help="")
        ...
        parser.add_argument('--raw-data-path', type=str, default="/root/ssd/DataSets/ImageNet/val_images", help="")
        ''' 
        ...
        try:
            ...
            img_siz = 256
        model = CauchyKesai(opt.bpu_siglip_weight_path, 1, 1)
        ...
        for cnt in tqdm(range(len(names)), desc=f"{s}: ", unit=" sample"):
            ...
            img_vec = model([preprocess(cv2.imread(img_path), img_siz).copy()])[0][0]
