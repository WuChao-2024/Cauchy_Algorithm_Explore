...
from transformers import SiglipVisionModel, SiglipProcessor, SiglipTokenizer
...
def main():
    ...
    for s in siglips:
        ...
        parser.add_argument('--img-vec-path', type=str, default=f"COCO2017_last_hidden_state_{s}.npz", help="")
        ...
        '''
        parser.add_argument('--raw-data-path', type=str, default="/home/users/chao01.wu/Datasets/COCO2017/val2017", help="")
        ''' 
        ...
        model = SiglipVisionModel.from_pretrained(opt.siglip_weight_path).to(device)
        names = os.listdir(opt.raw_data_path)
        img_vecs = {}
        for cnt in tqdm(range(len(names)), desc=f"{s}: ", unit=" sample"):
            name = names[cnt]
            img_path = os.path.join(opt.raw_data_path, name)
            with torch.no_grad():
                last_hidden_state = model(preprocess(cv2.imread(img_path), model.config.image_size).to(device)).last_hidden_state.detach().cpu().numpy()
                img_vecs[name] = last_hidden_state
